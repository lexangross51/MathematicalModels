...
public class LOS : IterativeSolver
{
    ...
    public override void Compute()
    {
        try
        {
            ...
            for (IterationsCount = 0; IterationsCount < MaxIters && squareNorm > Eps; IterationsCount++)
            {
...
public class CGMCholesky : IterativeSolver
{
    ...
    public override void Compute()
    {
        ...
        double[] ggNew = new double[Matrix.Gg.Length];
        Array.Copy(Matrix.Di, diNew, Matrix.Size);
        Array.Copy(Matrix.Gg, ggNew, Matrix.Gg.Length);
